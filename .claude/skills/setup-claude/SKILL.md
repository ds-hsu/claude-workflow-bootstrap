...
| 區塊 | 內容 |
|---|---|
| `permissions.deny` | 17 條破壞性指令（rm -rf、force push、reset --hard、sudo、kubectl delete、docker volume rm…） |
| `permissions.ask` | 17 條須確認指令（rm、git checkout --、docker exec、kubectl apply、npm install -g、psql/mysql/mongo/redis-cli 等 DB CLI…） |
...
- 陣列 permissions.deny 新增 17 筆
- 陣列 permissions.ask 新增 17 筆
```
...
- [ ] `permissions.deny` 含本範本 17 條規則
- [ ] `permissions.ask` 含本範本 17 條規則
